...
# Tu inicialización de Firebase ya existente...
...
#         texts_by_filename[filename] = text_list  # Almacenando los textos extraídos
#         return jsonify({'upload_image': filename, 'texts': text_list, 'firebase_url': public_url}), 200
MAX_REPEAT_COUNT = 2  # Definir el límite de veces que una placa puede repetirse
        ...
        if text_list:
            for plate_text in text_list:
                ...
                if plate_text not in entradas:
                    entradas[plate_text] = {'count': 1, 'last_entry_time': current_time}
                else:
                    if entradas[plate_text]['count'] >= MAX_REPEAT_COUNT:
                        entradas[plate_text] = {'count': 1, 'last_entry_time': current_time}
                    else:
                        entradas[plate_text]['count'] += 1
                        time_difference = (current_time - entradas[plate_text]['last_entry_time']).total_seconds() / 60.0
                        entradas[plate_text]['last_entry_time'] = current_time
                        entradas[plate_text]['time_spent'] = time_difference  # Guardar el tiempo pasado en el diccionario
                        return jsonify({'message': 'esto fue lo que duro el vehiculo en el estacionamiento', 'time_difference': time_difference})

        return jsonify({'upload_image': filename, 'texts': text_list, 'firebase_url': public_url}), 200
@app.route('/api/actualizar-placa', methods=['POST'])
def actualizar_placa():
    data = request.get_json()
    placa_original = data.get('placa_original')
    ...
    if placa_original in entradas:
        ...
        return jsonify({'message': 'Placa actualizada con éxito'}), 200
    else:
        return jsonify({'error': 'Placa original no encontrada'}), 404
...
def obtener_entradas():
    ...
    for placa, entrada in entradas.items():
        entrada_info = {
            ...
            'editado': entrada.get('editado', False)  # Asegurarse de incluir el estado de edición
        }
        if 'time_spent' in entrada:
            entrada_info['time_spent'] = entrada['time_spent']
...
def calcular_tarifa_endpoint():
    try:
        ...
        placa = data['placa']
        doc_ref = db.collection('entries').document(placa)
        doc = doc_ref.get()
        if doc.exists:
            entrada = doc.to_dict()
...
def upload_image():
    ...
    if file:
        ...
        if text_list:
            ...
            for plate_text in text_list:
                current_time = datetime.datetime.now()
                doc_ref = db.collection('entries').document(plate_text)
                doc = doc_ref.get()
                if not doc.exists:
                    doc_ref.set({
                        'count': 1,
...
def actualizar_placa():
    ...
    doc_ref = db.collection('entries').document(placa_original)
    doc = doc_ref.get()
    if doc.exists():
        entrada = doc.to_dict()
        entrada['editado'] = True
        db.collection('entries').document(nueva_placa).set(entrada)
        doc_ref.delete()
